...
<!-- Paginación -->
    <nav aria-label="Page navigation " class="pb-4">
        <ul class="pagination">
            {if $currentPage > 1}
                <li class="page-item">
                    <a class="page-link" href="{$shopName}/category/{$category->id_categoria}?page={$currentPage-1}">Anterior</a>
                </li>
            {/if}
            
            {section name=page start=1 loop=$totalPages+1}
                <li class="page-item {if $currentPage == $smarty.section.page.index}active{/if}">
                    <a class="page-link" href="{$shopName}/category/{$category->id_categoria}?page={$smarty.section.page.index}">{$smarty.section.page.index}</a>
                </li>
            {/section}

            {if $currentPage < $totalPages}
                <li class="page-item">
                    <a class="page-link" href="{$shopName}/category/{$category->id_categoria}?page={$currentPage+1}">Siguiente</a>
                </li>
            {/if}
        </ul>
    </nav>
