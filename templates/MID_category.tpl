...
<body>
   <main class="ini_mainpage_main mt-4">
   <div class="cm_categorypage-miniheader">
   <div class="cm_categorypage-categorypointer">
     <small id="current-location" class="cm_categorypage-current">Productos de la categoria <strong>"{$category->nombre}"</strong></small>
   </div>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    function openModal(image, title, price, description, id, stock) {
        ...
        quantityInput.value = stock > 0 ? 1 : 0; // Si hay stock, empieza en 1, si no, en 0
        quantityInput.max = stock; // Limita el máximo al stock disponible
        quantityInput.disabled = stock === 0; // Deshabilita el input si no hay stock
        if (stock < 1 && stock != null) {
            addToCartButton.disabled = true; // Deshabilitar el botón
            addToCartButton.innerText = "Sin stock"; // Cambiar el texto
        }else{
            addToCartButton.disabled = false; // Deshabilitar el botón
            addToCartButton.innerText = "Agregar al carrito"; // Cambiar el texto
        }
    ...
    document.querySelectorAll(".ini_mainpage_producto img").forEach(function(img) {
        img.addEventListener("click", function() {
            ...
            openModal(this.src, title, price, description, id, stock);
        });
    });
...
increaseBtn.addEventListener("click", function(event) {
    ...
    if (quantity < maxStock) { // No permitir superar el stock
        quantityInput.value = quantity + 1;
    }
});
